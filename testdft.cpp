...
//   DFT usage:
//   dft_func(complex<Type> *data,complex<Type> *out,int N,int Product,Type pi,int sign,int init)
//   dftinv_func(complex<Type> *data,complex<Type> *out,int N,Type pi,int init)
//
//   Product always set to 1
//   sign is 1 for forward dft, -1 for inverse (dftinv_func simply call dft_func with sign -1)
//   init set to 1 to compute roots of unity; 0 not compute (when calling dft_func/dftinv_func consecutively with same N)   
...
int main(int argc, char *argv[]) {
  ...
  if(1 == 1) {
    ...
    printf("test1\n");
    dft_func<double>(data_d,result1_d,24,1,pi_d,1,1);
    ...
    dftinv_func<double>(result1_d,result2_d,24,pi_d,0);    // no need to compute roots again
  ...
  if(1 == 0) {
    ...
    printf("test2\n");
    dft_func<float>(data_f,result1_f,24,1,pi_f,1,1);
    
    printf("TRANSFORM\n");  
    for(n=0;n<24;n++) {
        result1_f[n].print();
    }
    dftinv_func<float>(result1_f,result2_f,24,pi_f,0);
    printf("INV TRANSFORM\n");
    for(n=0;n<24;n++) {
        result2_f[n].print();
    }
  ...
  if(1 == 0) {
    ...
    printf("test3\n");
    dft_func<double>(data_d,result1_d,i,1,pi_d,1,1);
    ...
    dftinv_func<double>(result1_d,result2_d,i,pi_d,0);
    printf("INV TRANSFORM\n");
    for(n=0;n<i;n++) {
        result2_d[n].print();
    }
  ...
  if(1 == 0) {
    ...
    printf("test4\n");
    dft_func<float>(data_f,result1_f,i,1,pi_f,1,1);
    ...
    dftinv_func<float>(result1_f,result2_f,i,pi_f,0);
    printf("INV TRANSFORM\n");
    for(n=0;n<i;n++) {
        result2_f[n].print();
    }
