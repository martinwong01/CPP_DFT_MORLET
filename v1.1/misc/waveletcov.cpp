...
int main(int argc, char *argv[]) {
    ...
    for(s=0;s<Sx;s++) {
        ...
        x = fread(cwt1[0][0],Nx*Sy*aligned_dim<complex<float>>(Ny)*sizeof(complex<float>),1,fp1);   // cwt1. Note the aligned_dim here

	if(strcmp(argv[1],argv[2]) == 0)
	    memcpy(cwt2[0][0],cwt1[0][0],Nx*Sy*aligned_dim<complex<float>>(Ny)*sizeof(complex<float>));	
	else
	    x = fread(cwt2[0][0],Nx*Sy*aligned_dim<complex<float>>(Ny)*sizeof(complex<float>),1,fp2);
    ...
    for(i=0;i<tiles;i++)
    for(s=0;s<Sx;s++)
        fwrite(data[year-startyear][month-1][i][s],Sy*sizeof(double),1,fp);
