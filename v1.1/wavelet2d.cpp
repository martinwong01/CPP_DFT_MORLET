...
//  param = 6.;
	...
	for(x=0;x<Nx;x++) {                                                 // each Nx
            ...
            for(y=0;y<Ny;y++) {
                yseries1[thread][y] = real(xtransform[y][s][x]);
                yseries2[thread][y] = imag(xtransform[y][s][x]);
	    }
	    ...
	    for(i=0;i<Sy;i++)
	    for(y=0;y<Ny;y++) {
                real(cwt1[x][i][y]) = (float)(real(transform_real[x][i][y]) - imag(transform_imag[x][i][y]));
                imag(cwt1[x][i][y]) = (float)(imag(transform_real[x][i][y]) + real(transform_imag[x][i][y]));
                real(cwt2[x][i][y]) = (float)(real(transform_real[x][i][y]) + imag(transform_imag[x][i][y]));
                imag(cwt2[x][i][y]) = (float)(real(transform_imag[x][i][y]) - imag(transform_real[x][i][y]));
            }
#elif AVX512F == 0
            for(i=0;i<Sy;i++)
            for(y=0;y<Ny;y+=2) {
