...
int main(int argc, char *argv[]) {
    ...
    for(s=0;s<Sx;s++) {
        ...
        for(k=0;k<Sy;k++)
        for(y=0;y<Ny;y++) {
            anom1[y][x] += real(cwt1[x][k][y])*scales_sqroot_inv[s][k];
            cov1[s][k] += real(cwt1[x][k][y]*conj(cwt1[x][k][y]));
            anom2[y][x] += real(cwt2[x][k][y])*scales_sqroot_inv[s][k];
            cov2[s][k] += real(cwt2[x][k][y]*conj(cwt2[x][k][y]));
        }
    }
    fclose(fp1);
    fclose(fp2);
