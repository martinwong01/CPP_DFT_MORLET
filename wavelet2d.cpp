...
//  param = 6.;
    ...
    #pragma omp parallel default(shared) private(x)
    {
        ...
        for(y=0;y<Ny;y++) {                                                     // xtransform for each y
            if(x < 0) x = y;                                                    // x keeps the 1st y of each thread 
            if(y == x) 
	        morlet<double>(data[y],xtransform[y],Nx,Sx,param,dx,pi,1);
            else
                morlet<double>(data[y],xtransform[y],Nx,Sx,param,dx,pi,0);      // don't compute constants/roots again
        }
    }
    ...
    for(s=0;s<Sx;s++) {                                                         // each Sx
        printf("s= %d\n",s);
        system("date");
        #pragma omp parallel default(shared) private(thread,y,i,j)
        {
            j = -999;
            #pragma omp for 
	    for(x=0;x<Nx;x++) {                                                 // each Nx
                ...
                if(x == j) 
                    morlet<double>(yseries1[thread],transform_real[x],Ny,Sy,param,dy,pi,1);
                else
